...
class ProductRepository:
    ...
    def featured_products() -> QuerySet[Product]:
        return Product.objects.select_related("category").prefetch_related("images", "size_chart").filter(active=True, featured=True).order_by("-created_at")

    @staticmethod
    def get_by_slug(slug: str):
        return Product.objects.select_related("category").prefetch_related("images", "size_chart").filter(active=True, slug=slug).first()
